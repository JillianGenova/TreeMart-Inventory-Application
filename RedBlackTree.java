...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
	...
	protected static class Node<T> {
		 ...
		 * RedBlackTree class below produces an in order traversal of the nodes / values
	...
	public boolean insert(T data) throws NullPointerException, IllegalArgumentException {
		...
		if (root == null) {
			root = newNode;
			size++; // add first node to an empty tree
			root.isBlack = true;
			return true;
		} else {
	 ...
	 *                should be inserted as a descendent beneath
	...
	/**
	 * The job of this enforceRBTreePropertiesAfterInsert method is to resolve a red
	 * child under a red parent red black tree property violations that are
	 * introduced by inserting new nodes into a red black tree. While doing so, all
	 * other red black tree properties will also be preserved.
	 * 
	 * @param newNode to insert to ensure the properties hold
	 */
	private boolean enforceRBTreePropertiesAfterInsert(Node<T> newNode) {
		if (newNode.parent.isBlack) {
			// no RB Tree properties are broken
			// if parent is root this will be true
			return true;
		} else { // parent is red
			// node grandparent exists since parent cannot be root
			// left parent has black sibling
			if (newNode.parent.isLeftChild()
					&& !(newNode.parent.parent.rightChild != null && !newNode.parent.parent.rightChild.isBlack)) {
				// violating node on opposite side (on left)
				if (newNode.isLeftChild()) {
					newNode.parent.isBlack = true;
					newNode.parent.parent.isBlack = false;
					rotate(newNode.parent, newNode.parent.parent);

				} else { // violating node on same side
				...
				// make sure parent of grandparent is black or null
				// if not, push swap up the tree
				Node<T> currentNode = newNode.parent.parent;
				if (currentNode != root && !currentNode.parent.isBlack) {
				...
				// sibling is red, parent on right
			} else if (!newNode.parent.isLeftChild()
					&& !(newNode.parent.parent.leftChild != null && newNode.parent.parent.leftChild.isBlack)) {
				// set parent and sibling to black
				newNode.parent.isBlack = true;
	...
	private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
		...
		if (child.isLeftChild()) { // perform a right rotation
			Node<T> oldParent = parent;
			Node<T> leftRightChild = child.rightChild;
			Node<T> grandparent = parent.parent;
			if (grandparent == null) { // parent is the root node
				this.root = child;
				child.parent = this.root;
			} else if (parent.isLeftChild()) { // parent is grandparent's left child
			...
			parent.parent = child;
			parent.leftChild = leftRightChild;
			if (leftRightChild != null) {
				leftRightChild.parent = parent;
			}

		} else { // perform a left rotation (child is on right)
			Node<T> oldParent = parent;
			Node<T> rightLeftChild = child.leftChild;
			Node<T> grandparent = parent.parent;
			if (grandparent == null) { // parent is the root node
				this.root = child;
				child.parent = this.root;
				...
				child.parent = grandparent;
				// child's parent, and make sure right children of child
			} // parent is still referring to old parent
			child.leftChild = oldParent;
			parent.parent = child;
			parent.rightChild = rightLeftChild; // parent is still parameter
			if (rightLeftChild != null) {
				rightLeftChild.parent = oldParent;
			}
		}
	}
	...
	public T get(T data, Node<T> subtree) {
		if (data == null)
			throw new NullPointerException("You didn't enter a valid product");
		if (subtree == null) {
			throw new NoSuchElementException("We don't currently sell that product");
		} else {
			int compare = data.compareTo(subtree.data);
			if (compare < 0) {
